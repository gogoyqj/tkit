 ...
 * @LastEditTime: 2019-12-20 12:40:34
 */

import { Tction, MM, useModel } from 'src/useModel';
import { UserModel } from './testMMOK';
...
export const UserFailModel = MM({
  ...
  effects: {
    doFetchName: async ({ tPut }, action: Tction<{ time: number }>): Promise<{}> => {
      ...
      return tPut(UserModel.actions.doRename, { username: action.payload.time });
    }
  }
});
