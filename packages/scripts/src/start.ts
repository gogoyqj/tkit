...
import { ensureFile, ensureFileModern, TkitAppDirectory } from 'tkit-utils';
...
// eslint-disable-next-line @typescript-eslint/no-var-requires
const configWebpackDevServer = require(ensureFileModern(
...
choosePort(HOST, DEFAULT_PORT)
  .then(port => {
    ...
    const proxyJS = path.join(TkitAppDirectory, 'config', 'proxy');
    const proxyConfig = fs.existsSync(proxyJS + '.js')
      ? require(proxyJS)
      : prepareProxy(proxySetting, paths.appPublic);
    // Serve webpack assets generated by the compiler over a web sever.
    const serverConfig: WebpackDevServer.Configuration = configWebpackDevServer(
      proxyConfig,
